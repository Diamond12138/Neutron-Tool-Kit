    ...
    namespace Graphics
    {
        ...
        bool TrueTypeFont::load_glyph(std::uint32_t code, std::uint32_t size)
        {
            ...
            if (i != m_glyphs.end())
            {
                ...
                {
                    Glyph glyph;
                    bool success = load_glyph(code, size, glyph);
                    i->second.insert(std::pair<std::uint32_t, Glyph>(code, glyph));
                    return success;
                }
            }
            else
            {
                Glyph glyph;
                bool success = load_glyph(code, size, glyph);
                std::map<std::uint32_t, Glyph> item;
                item.insert(std::pair<std::uint32_t, Glyph>(code, glyph));
                m_glyphs.insert(std::pair<std::uint32_t, std::map<std::uint32_t, Glyph>>(size, item));
                return success;
            }
        }
    } // namespace Graphics
